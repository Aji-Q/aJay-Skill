...
| 强制老路径 (保险) | `AJAY_LEGACY=1 python run.py <ticker>` |
...
| 版本 | 变化 | 影响 agent 的部分 |
|---|---|---|
| v3.0.0 | pipeline 默认启用 · `AJAY_LEGACY=1` 回老路径 | `python run.py` 默认进 pipeline |
...
aJay-Skill 既可能在**中国大陆**运行，也可能在 **Codex / 海外云容器**里运行，

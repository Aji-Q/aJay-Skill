# J Trader · Claude Code Context

> 本文件供 Claude Code 自动读取，提供项目上下文。

## 这是什么

J Trader 是由 aJay（Aji-Q）维护的美股研究 plugin（保留 A 股 / 港股兼容路径）。技术插件名继续使用 `ajay`。用户说“用 J Trader 分析 XXX”时，你应该自动触发 `deep-analysis` skill。

## 核心技能

| Skill | 触发条件 | 说明 |
|---|---|---|
| `deep-analysis` | 用户提到"分析/研究/估值/DCF/值不值得买"等 | 多维数据 + 市场适配评审团 + J Trader 编辑式报告 |

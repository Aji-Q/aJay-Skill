...
1. **`run.py` 在 repo root · 不在 `scripts/` 下**
   - ✅ `python run.py <ticker>`（从 aJay-Skill/ 目录）
...
AJAY_LEGACY=1 python run.py <ticker>   # 强制走老 stage1/stage2 (保险绳)
```

**不要删 `rrt.collect_raw_data` / `rrt.stage1` / `rrt.stage2`** · 它们是 AJAY_LEGACY 的 fallback 路径 · 删了就是破坏保险.
...
```bash
cd <repo-root>          # 例如 ~/code/aJay-Skill 或 git clone 到的任意路径
...
| "rrt.collect_raw_data 是死代码" | 是 `AJAY_LEGACY=1` 的 fallback · 保留 |

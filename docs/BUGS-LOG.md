...
- **修法**：
  ...
  4. `_cninfo_disclosures` 优先调直连 API · 直连失败时**默认不调 akshare**（防长尾）· 仅 `AJAY_AK_CNINFO_FALLBACK=1` 显式启用时才走 akshare 慢路径
- **验证**：
...
- **位置**：Hermes Skills Guard 模式匹配扫描器（NousResearch/hermes-agent · 上游 bug · 不是 aJay-Skill 的问题）
...
- **改动**：
  ...
  3. JS 初始化优先级：`localStorage.getItem('ajay-theme')` > `prefers-color-scheme: dark` > light
...
- **改动**：
  1. `run.py` argparse 新增 `--school` choices=[A,B,C,D,E,F,G] · 触发后 set `os.environ["AJAY_SCHOOL"]`
...
- **验证**：
  - `AJAY_SCHOOL=F python3 -c "from lib.investor_evaluator import evaluate; print(evaluate('buffett', {...}))"` → signal=skip · reason 含"锁定 A 股游资 派视角"
  - `_render_school_lock_banner({"school_lock":{"group":"F","label":"A 股游资"}})` → 包含 "SCHOOL LOCK" + "赵老哥" + 深红配色
  - 11 个回归测试 (`test_v3_5_0_school_lock.py`) 全过
- **未来改该区域注意事项**：
  - `AJAY_SCHOOL` env 大小写归一在 `get_locked_school()` 内完成 · 不要在其他位置再做 upper() 否则双重转换会失效
...
### FEATURE · `--output-dir` CLI flag · 供 ajay-platform 集成
- **背景**：把 aJay-Skill 包装成 SaaS（`ajay-platform/`），后端 Celery worker 需要稳定产物路径挂载到 web 报告页
...
- **安全设计**：
  ...
  - 非交互环境必须 `AJAY_FUND_AUTO_YES=1` 显式确认
...
- **未来改该区域注意事项**：
  ...
  - `AJAY_FUND_AUTO_YES=1` 应用于 CI / agent 编排场景 · 不要默认开启
...
- **修法**（多重 layer）：
  1. **显式 escape hatch**：`AJAY_DISABLE_MINI_RACER=1` env var · 跳过 3 个 fetcher graceful 降级
  2. **自动恢复**（核心创新）：sentinel 文件机制
     - 调 mini_racer fetcher 前写 `~/.ajay-skill/_minirackercrash.sentinel`
     - 成功后删
     - 进程崩则 sentinel 留下 · 下次启动自动 disable
  3. **强制启用**：`AJAY_FORCE_MINI_RACER=1` 覆盖 sentinel（debug 用）
  4. legacy + pipeline 两条路径都做了同样保护
- **验证**：AJAY_DISABLE_MINI_RACER=1 e2e 跑通 · 614 KB HTML 仍生成
- **回归测试**：`tests/test_v3_3_4_minirackerguard.py` (7 tests)
- **未来改该区域注意事项**：
  - 不要试图用 Python `try/except` 抓 V8 SIGTRAP · 抓不到（进程级 signal）
  - sentinel 文件路径在 `~/.ajay-skill/_minirackercrash.sentinel` · 不要改路径
...
- **修法**（3 步）：
  ...
  3. `run.py::main` 默认走 pipeline · `AJAY_LEGACY=1` 强制走老路径 · pipeline 异常自动 fallback · 附 traceback
...
- **未来改该区域注意事项**：
  ...
  - `AJAY_LEGACY=1` 是保险开关 · 绝不删 · 出问题时用户能临时回退
...
- **设计要点**：
  ...
  - 非 TTY / `AJAY_NO_UPDATE_CHECK=1` / 网络异常 全部 silent skip · 不阻塞主流程
...
- **未来改该区域注意事项**：
  - `AJAY_NO_UPDATE_CHECK=1` 在 CI / Codex 环境必须默认设 · 防阻塞
...
- **附加**：
  - 加 `AJAY_PLAYWRIGHT_FORCE=1` 环境变量 · 用户强制 kill switch · 忽略 quality 判定
...
- **若未来改 Playwright 触发**：
  ...
  - `AJAY_PLAYWRIGHT_FORCE` 是用户 kill switch · 不能移除
...
- **策略**（Codex review 后收敛）：
  ...
  | medium | opt-in · 需 `AJAY_PLAYWRIGHT_ENABLE=1` | 4 维（4_peers/8_materials/15_events/17_sentiment） | 未装时打印命令让用户手动装，本次跳过 |
...
- **修法**（三层 fallback + 一层保底）：
  ...
  3. Tier 3 · 雪球 Playwright 登录态（用户 opt-in `AJAY_XQ_LOGIN=1`）· 复用 `lib/xueqiu_browser.py::fetch_with_browser` + 新加 `fetch_peers_via_browser(code)`
...
### BUG · `run.py` 直跑模式未自动标记 AJAY_CLI_ONLY
- **症状**：`python run.py 002273.SZ --depth medium` → `agent_analysis` 缺失仍 critical → block HTML
- **位置**：`run.py::main()` 环境变量设置区
- **根因**：CLI 降级逻辑依赖 `AJAY_DEPTH=lite / AJAY_LITE=1 / AJAY_CLI_ONLY=1 / CI=true` 四个信号；medium 模式都不命中
- **修法**：run.py main() 开头加 `os.environ.setdefault("AJAY_CLI_ONLY", "1")` — run.py 是 CLI 直跑入口（agent 流程走 stage1/stage2 直接调用，不经 run.py）
- **验证**：002273.SZ medium → HTML 生成 ✅
- **回归测试**：`test_v2_10_4_fixes.py::test_run_py_sets_cli_only_env`
- **若未来改 run.py**：不能删 AJAY_CLI_ONLY=1 setdefault；若新增 agent 专用入口必须另设标志区分
...
- **症状**：`AJAY_DEPTH=lite` 跑完 gate 报 9 个 critical（维度缺失、data 为空）
...
- **修法**：`AJAY_DEPTH=lite / AJAY_LITE=1 / AJAY_CLI_ONLY=1 / CI=true` 任一命中 → 降 warning
...
- **修法**：
  ...
  - `fetch_contests` HTTP fail → 检查 AJAY_XQ_LOGIN → Playwright fallback
...
- **修法**：fetch_fund_holders 默认 `AJAY_FUND_WORKERS=1`（serial）；同样修
  `lib/quant_signal.py` 内部并发 → 默认 `AJAY_QUANT_WORKERS=1`

...
aJay-Skill 采用多数据源 + 自动 failover 架构 (v2.10.3 起)。本文档列出**所有能接入**的 providers 及配置方法。
...
AJAY_PROVIDERS_KLINE=baostock,akshare python -m lib.providers chain A kline
```
...
可用 `AJAY_PROVIDERS_<DIM>` 覆盖单维度优先级，如：
```bash

...
> **aJay** is a derivative of [stock-deep-analyzer 3.9.4](https://github.com/wbh604/UZI-Skill) (MIT, by Float Future): renamed, upstream update-check removed, plus a US-market backfill chain (SEC XBRL financials & valuation quantiles / moomoo capital flow / czsc Chan-theory structure / FMP consensus). Upstream history: `docs/UPSTREAM-RELEASE-NOTES.md`; aJay changes: `RELEASE-NOTES.md`.
...
```
/ajay:analyze-stock 600519         # Kweichow Moutai (A-share)
/ajay:analyze-stock 00700.HK       # Tencent (HK)
/ajay:analyze-stock BABA           # Alibaba ADR
/ajay:analyze-stock AAPL           # Apple
```
...
Then say `/ajay:analyze-stock Tencent` or `/ajay:analyze-stock 00700.HK`.

> ⚠️ **Always use the `ajay:` namespace prefix**
>
> After install, all skills/commands live under `ajay:<name>`. Short names (`/analyze-stock`) don't always resolve in every environment — to be safe, always use the full name:
> - `/ajay:analyze-stock <ticker>`
> - `/ajay:quick-scan <ticker>`
> - `/ajay:scan-trap <ticker>`
> - `/ajay:dcf <ticker>` / `:ic-memo` / `:investor-panel` / `:trap-detector` / ...
>
...
> Please follow https://raw.githubusercontent.com/wbh604/UZI-Skill/main/.codex/INSTALL.md to install aJay-Skill, then deep-analyze Alibaba (BABA).
...
cd aJay-Skill && pip install -r requirements.txt
...
```
/ajay:analyze-stock 600519          # A-share by ticker
/ajay:analyze-stock 00700.HK        # HK
/ajay:analyze-stock BABA            # US ADR
/ajay:analyze-stock AAPL            # US
```
...
> All commands require the `/ajay:` prefix for reliable dispatch across environments.

| Command | What It Does |
|---|---|
| `/ajay:dcf 600519` | DCF valuation · WACC + 5×5 sensitivity table |
| `/ajay:comps 002273` | Peer comparison · PE / PB percentile ranking |
| `/ajay:lbo 600519` | LBO stress test · PE-buyer IRR perspective |
| `/ajay:initiate BABA` | Initiating-coverage report · JPM / GS format |
| `/ajay:ic-memo BABA` | Investment-committee memo · 3-scenario returns |
| `/ajay:earnings AAPL` | Earnings beat/miss analysis |
| `/ajay:catalysts 300750` | Catalyst calendar · next 60 days |
| `/ajay:thesis 600519` | Investment-thesis tracker · 5 pillars |
| `/ajay:screen AAPL` | 5 quant screens · value / growth / quality / GARP / short |
| `/ajay:dd BABA` | Due-diligence checklist · 21 items across 5 workstreams |
| `/ajay:quick-scan 00700.HK` | 30-second sanity check |
| `/ajay:panel-only 600519` | Just run the 66-investor jury — no HTML report |
| `/ajay:scan-trap 600519` | Pump-and-dump / trap detection (8 signals) |
| `/ajay:segmental-model 300308` | 🆕 Bottom-up segmental revenue model · 3-scenario × 3-year projection · cross-checks top-down DCF |
...
```bash
export AJAY_DEPTH=lite       # or medium / deep
...
| `/ajay:analyze-stock 600519` | medium (default) |
...
|---|---|---|
| **XueQiu** (`cubes_search`) | Dim 19 · real-money portfolios holding this stock + their returns | `export AJAY_XQ_LOGIN=1 && python -m lib.xueqiu_browser login` (one-time browser prompt; cookies persisted to `~/.ajay-skill/playwright-xueqiu/`). Or use flag: `python run.py BABA --enable-xueqiu-login`. |
...
A: Yes. `/ajay:analyze-stock AAPL`, `/ajay:analyze-stock BABA`, `/ajay:analyze-stock 00700.HK`. HK now has 4-layer kline fallback (Eastmoney → Sina → yfinance → Yahoo Chart v8) since v2.13.7.
...
> - `python run.py <ticker>` now defaults to `pipeline.run_pipeline` · `AJAY_LEGACY=1` falls back

(() => {
  ...
  function renderDossier(){
    ...
    if(state.dossierTab==='evidence'){
      ...
      [['aJay 的私人研究室','本项目由 aJay / Aji-Q 维护。城市只改变视觉场景，不改变数据、议题或当前结论。'],['方法，而非人物背书',data.council.disclosure],['证据优先','议题观点由原始输入确定生成，不使用旧 persona 的随机台词，也不把缺失值当成零。规则分、角色一致度与数据覆盖均不是上涨概率；尚未完成样本外校准。'],['图像来源','人物为 AI 生成的编辑肖像。城市图片的具体来源、日期与编辑记录见下方逐图说明。'],['软件来源','aJay 的新增设计与实现保留署名；MIT 上游作者与贡献历史保存在 LICENSE / NOTICE。GSAP 与 Lucide 依其各自许可使用。']].forEach(([h,p])=>{box.append(el('h3','',h),el('p','',p));});
      ...
      const link=el('a','','Aji-Q / aJay-Skill');link.href='https://github.com/Aji-Q/aJay-Skill';link.target='_blank';link.rel='noopener noreferrer';box.append(el('p','note','本报告不构成投资建议。'),link);content.append(box);
    }
    content.scrollTop=0;
  }
  ...
  $('download-inputs').addEventListener('click',()=>{const blob=new Blob([JSON.stringify(data.raw,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=el('a');a.href=url;a.download=`${String(data.security.ticker||'aJay').replace(/[^a-zA-Z0-9_.-]/g,'_')}-inputs.json`;document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);});

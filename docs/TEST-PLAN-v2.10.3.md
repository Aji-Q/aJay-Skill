...
```bash
# A 股测试：茅台 lite 模式
python run.py 600519 --depth lite --no-browser 2>&1 | tee /tmp/ajay_test_600519_lite.log | tail -50
```
...
```bash
python run.py 512400 --no-browser 2>&1 | tee /tmp/ajay_test_etf.log | head -30
```
...
import warnings; warnings.filterwarnings('ignore')
import os
os.environ['AJAY_FUND_STATS_TOP'] = '5'  # 只算 5 家 full，其他 lite
...
**如果某个 fetcher 一直挂**：
```bash
export AJAY_HTTP_TIMEOUT=10     # 短超时快速 fail
export AJAY_DDG_TIMEOUT=5
...
**如果想强制某个 provider**：
```bash
export AJAY_PROVIDERS_FINANCIALS=tushare,akshare
python run.py <ticker>
```

**如果想关掉预检（对比有无）**：
```bash
export AJAY_SKIP_PREFLIGHT=1
python run.py <ticker>
```

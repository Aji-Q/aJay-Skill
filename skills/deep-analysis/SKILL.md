...
环境变量 `AJAY_SCHOOL` 会被设置 · synthesis.json 里 `school_lock` 字段也会标注。

**进入 stage1 后的 role-play 阶段 · 你必须**：

1. **读 `panel.json` 里 group == AJAY_SCHOOL 的评委**（通常 5-8 人）· 只 role-play 这些人
...
**绝不能**：
- ❌ 不顾 `AJAY_SCHOOL` 把 65 人都 role-play 一遍（其他派 skip 状态会被你的 override 覆盖 · 误导用户）
- ❌ 在 `panel_insights` 里写"价值派看空但游资看多"这种跨派叙事（用户已选了一派 · 不需要外部对比）

何时 skip 本 HARD-GATE：
- `AJAY_SCHOOL` 未设置（默认 · 全 65 评委正常 role-play）
...
```python
import os
os.environ["AJAY_PLAYWRIGHT_FORCE"] = "1"
...
**绕过方式**（仅 lite / CI 环境）：
- `AJAY_DEPTH=lite` · lite 模式 `playwright_mode=off` · 此 HARD-GATE 自动跳过
- `AJAY_PLAYWRIGHT_ENABLE=0` · 显式禁用（但 deep 档不建议）
...
**绝不能**：
- 看到 exit 1 就 export AJAY_SKIP_REVIEW=1 绕过（那是调试用的，生产发 HTML 给用户绝不该用）
...
非 Claude 平台跑 aJay-Skill 时常见问题已被代码层修复，但 agent 也要主动适配：
...
`AJAY_DISABLE_GLOBAL_PEERS=1` 关闭，或用 `AJAY_GLOBAL_PEER_LIMIT=3..12` 调整补全数量。

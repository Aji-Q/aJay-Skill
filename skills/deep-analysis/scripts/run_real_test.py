...
# 用户可设 `AJAY_DISABLE_MINI_RACER=1` env var 完全跳过这 3 个 fetcher · graceful 降级
...
_MINI_RACER_SENTINEL = Path.home() / ".ajay-skill" / "_minirackercrash.sentinel"


def _mini_racer_disabled() -> bool:
    """v3.3.4 · 多重判断是否禁用 mini_racer.

    1. 显式 env var `AJAY_DISABLE_MINI_RACER=1` → disable
    2. `AJAY_FORCE_MINI_RACER=1` → force enable (overrides sentinel)
    3. sentinel 文件存在（上次 crash 留下）→ auto-disable + 提示
    """
    if os.environ.get("AJAY_DISABLE_MINI_RACER") == "1":
        return True
    if os.environ.get("AJAY_FORCE_MINI_RACER") == "1":
        return False
    if _MINI_RACER_SENTINEL.exists():
        # auto-recovery：上次 mini_racer 崩过 · 本次自动 skip
        # 用户想重试可 `rm ~/.ajay-skill/_minirackercrash.sentinel` 或 AJAY_FORCE_MINI_RACER=1
        if not getattr(_mini_racer_disabled, "_warned", False):
            ...
            print(
                f"   ℹ️  想重试 · `rm {_MINI_RACER_SENTINEL}` 或 `AJAY_FORCE_MINI_RACER=1`",
                file=sys.stderr,
            )
...
def collect_raw_data(ticker: str, max_workers: int = 6, resume: bool = True) -> dict:
    ...
    are always re-fetched. Use `resume=False` (or env AJAY_NO_RESUME=1) to
    force full re-fetch.
    """
    # v2.6 · 允许通过 env 关闭 resume（run.py --no-resume 设置）
    if os.environ.get("AJAY_NO_RESUME") == "1":
        resume = False
    ...
    def _fund_holders():
        try:
            ...
            # AJAY_FUND_STATS_TOP=N 控制几家算完整业绩（默认 20）。
...
def _detect_lite_mode() -> tuple[bool, str]:
    ...
    import os
    from pathlib import Path
    val = os.environ.get("AJAY_LITE", "auto").lower()
    if val in ("1", "true", "yes", "on"):
        return True, "AJAY_LITE=1 显式启用"
    if val in ("0", "false", "no", "off"):
        return False, "AJAY_LITE=0 显式关闭"
...
def stage1(ticker: str) -> dict:
    ...
    # lite: 完全禁用 · medium: opt-in (AJAY_PLAYWRIGHT_ENABLE=1) · deep: 默认启用（未装 y/n 确认装）
    try:
...
def stage2(ticker: str) -> str:
    ...
    if os.environ.get("AJAY_NO_AUTO_OPEN") != "1":
        try:
            import webbrowser

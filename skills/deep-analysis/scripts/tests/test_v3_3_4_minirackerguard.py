...
修法（多重）:
1. `AJAY_DISABLE_MINI_RACER=1` env var · 显式跳过 3 个 fetcher · graceful 降级
2. Sentinel 文件 (~/.ajay-skill/_minirackercrash.sentinel) · 调用前 arm · 成功后 disarm
   下次启动若 sentinel 还在 = 上次崩了 · 自动 disable
3. `AJAY_FORCE_MINI_RACER=1` 强制启用（让用户能手动测试 / 强制忽略 sentinel）
"""
...
def _clean_state(monkeypatch, tmp_path):
    """每个测试隔离 env + sentinel."""
    for k in ("AJAY_DISABLE_MINI_RACER", "AJAY_FORCE_MINI_RACER"):
        monkeypatch.delenv(k, raising=False)
    sentinel = tmp_path / ".ajay-skill" / "_minirackercrash.sentinel"
    return sentinel


def test_disable_env_var_skips_fetcher(monkeypatch, tmp_path):
    """AJAY_DISABLE_MINI_RACER=1 时 · 跑 fetch_industry 应直接跳过返 fallback."""
    import run_real_test as rrt

    monkeypatch.setenv("AJAY_DISABLE_MINI_RACER", "1")
...
def test_force_overrides_sentinel(monkeypatch, tmp_path):
    """AJAY_FORCE_MINI_RACER=1 时 · 即使 sentinel 存在也应启用 mini_racer."""
    import run_real_test as rrt
    ...
    monkeypatch.delenv("AJAY_DISABLE_MINI_RACER", raising=False)
    monkeypatch.setenv("AJAY_FORCE_MINI_RACER", "1")

    # 重置 _warned · 避免之前测试污染
    if hasattr(rrt._mini_racer_disabled, "_warned"):
        del rrt._mini_racer_disabled._warned

    assert rrt._mini_racer_disabled() is False, "AJAY_FORCE_MINI_RACER=1 应覆盖 sentinel"
...
def test_sentinel_auto_disable(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("AJAY_DISABLE_MINI_RACER", raising=False)
    monkeypatch.delenv("AJAY_FORCE_MINI_RACER", raising=False)
...
def test_pipeline_collect_respects_disable(monkeypatch):
    """pipeline.collect 也应识别 AJAY_DISABLE_MINI_RACER · 不调 fetcher."""
    src = (SCRIPTS / "lib" / "pipeline" / "collect.py").read_text(encoding="utf-8")
    assert "AJAY_DISABLE_MINI_RACER" in src, (
        "pipeline.collect 必须支持 AJAY_DISABLE_MINI_RACER · 否则 pipeline 路径仍会崩 (issue #61)"
    )

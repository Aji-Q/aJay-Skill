...
def test_xueqiu_browser_opt_in_only():
    ...
    assert "AJAY_XQ_LOGIN" in src, "BUG#R5 regression: 必须用 AJAY_XQ_LOGIN env 启用"
...
def test_fund_holders_two_tier_strategy():
    ...
    assert "AJAY_FUND_STATS_TOP" in src, \
        "v2.10.1 regression: 必须支持 AJAY_FUND_STATS_TOP 环境变量控制几家算完整业绩"
...
def test_lite_mode_detection_exists():
    """v2.10.1 · _detect_lite_mode 必须存在（v3.1 · AJAY_LITE / AJAY_DDG_BUDGET 在 preflight_helpers）"""
    src = (
        (SCRIPTS_DIR / "run_real_test.py").read_text(encoding="utf-8")
        + "\n" + (SCRIPTS_DIR / "lib" / "pipeline" / "score_fns.py").read_text(encoding="utf-8")
        + "\n" + (SCRIPTS_DIR / "lib" / "pipeline" / "preflight_helpers.py").read_text(encoding="utf-8")
    ...
    assert "AJAY_LITE" in src
    assert "AJAY_DDG_BUDGET" in src


def test_ddg_budget_enforced():
    """v2.10.1 · search() 必须在超预算时返回 _budget_exceeded 标记并过滤掉"""
    import os
    os.environ["AJAY_DDG_BUDGET"] = "0"  # 强制超预算
    try:
        ...
        os.environ.pop("AJAY_DDG_BUDGET", None)
...
def test_analysis_profile_env_compat():
    """AJAY_LITE=1 必须向后兼容到 depth=lite"""
    import os
    from lib.analysis_profile import get_profile, DEPTH_LITE, DEPTH_MEDIUM
    # AJAY_LITE=1
    os.environ["AJAY_LITE"] = "1"
    os.environ.pop("AJAY_DEPTH", None)
    assert get_profile().depth == DEPTH_LITE
    # 显式 AJAY_DEPTH 覆盖 AJAY_LITE
    os.environ["AJAY_DEPTH"] = "medium"
    assert get_profile().depth == DEPTH_MEDIUM
    # 清理
    os.environ.pop("AJAY_LITE", None)
    os.environ.pop("AJAY_DEPTH", None)
...
def test_ddg_timeout_wrapper():
    ...
    assert "AJAY_DDG_TIMEOUT" in src, "v2.10.2 regression: DDGS 缺 timeout env"
...
def test_net_timeout_guard_exists():
    ...
    assert "AJAY_HTTP_TIMEOUT" in src
...
def test_provider_env_override():
    """AJAY_PROVIDERS_<DIM> 环境变量可覆盖优先级"""
    import os
    from lib import providers
    os.environ["AJAY_PROVIDERS_FINANCIALS"] = "baostock,akshare"
    try:
        chain = providers.get_provider_chain("financials", market="A")
        ...
        os.environ.pop("AJAY_PROVIDERS_FINANCIALS", None)
...
def test_fetch_industry_respects_lite_mode():
    """v2.10.1 · fetch_industry 在 AJAY_LITE=1 时不跑 _dynamic_industry_overview"""
    src = (SCRIPTS_DIR / "fetch_industry.py").read_text(encoding="utf-8")
    assert 'AJAY_LITE' in src and '_dynamic_industry_overview' in src
    # 检查早退逻辑
    idx = src.find('AJAY_LITE')

"""v3.0.0 Phase 7 · run.py AJAY_PIPELINE=1 opt-in 集成测试."""
...
def test_run_py_has_uzi_pipeline_check():
    """run.py 含 AJAY_PIPELINE=1 检测 · Phase 7 的核心."""
    run_py = ROOT / "run.py"
    txt = run_py.read_text(encoding="utf-8")
    assert "AJAY_PIPELINE" in txt, "run.py 必须检测 AJAY_PIPELINE env"
...
def test_run_py_default_still_legacy():
    """AJAY_PIPELINE 未设时必须保持 legacy 路径 · 向后兼容."""
    run_py = ROOT / "run.py"
    txt = run_py.read_text(encoding="utf-8")
...
def test_pipeline_imports_work():
    """AJAY_PIPELINE=1 入口 · import 链路完整 · 不会因为循环 import 挂."""

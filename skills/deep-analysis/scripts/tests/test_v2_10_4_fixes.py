...
def isolate_profile_environment(monkeypatch):
    ...
    for key in ("CI", "AJAY_DEPTH", "AJAY_LITE", "AJAY_CLI_ONLY"):
...
def _reset_profile_env():
    for k in ("AJAY_DEPTH", "AJAY_LITE", "AJAY_CLI_ONLY"):
...
def test_run_py_sets_cli_only_env():
    """Fix (v2.10.5) · run.py is CLI direct entrypoint · should auto-set AJAY_CLI_ONLY=1.
    ...
    assert 'os.environ.setdefault("AJAY_CLI_ONLY", "1")' in src, (
        "run.py must set AJAY_CLI_ONLY=1 so CLI direct runs produce HTML"
    )

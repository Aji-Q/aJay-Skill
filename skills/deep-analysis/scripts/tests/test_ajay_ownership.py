"""aJay identity belongs at active entrypoints; upstream provenance stays intact."""
...
def test_distribution_metadata_has_one_current_version(relative):
    ...
    assert "aJay" in data["description"]
    if "author" in data:
        author = data["author"]
...
def test_secondary_report_brand_and_disclaimer_belong_to_ajay():
    for relative in ("lib/portfolio_runner.py", "lib/versus_runner.py"):
        content = (SCRIPTS / relative).read_text()
        assert "Generated by aJay" in content
        assert f"aJay-Skill v{VERSION}" in content
        assert "FloatFu-true" not in content
    disclaimer = (ROOT / "skills/deep-analysis/assets/disclaimer.md").read_text()
    assert "aJay Research · aJay" in disclaimer
    assert "FloatFu-true" not in disclaimer


def test_session_hook_reports_current_ajay_identity_without_network(monkeypatch):
    ...
    assert f"aJay Research v{VERSION}" in context
...
def test_secondary_report_outputs_render_with_current_identity(tmp_path, monkeypatch):
    ...
    for html in (portfolio, versus):
        assert "Generated by aJay" in html
        assert f"aJay-Skill v{VERSION}" in html

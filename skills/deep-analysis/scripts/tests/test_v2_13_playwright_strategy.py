...
- medium: playwright_mode='opt-in' · AJAY_PLAYWRIGHT_ENABLE=1 才启用 · 未装只打命令
...
def _reset_env():
    for k in ("AJAY_DEPTH", "AJAY_LITE", "AJAY_PLAYWRIGHT_ENABLE"):
...
def test_is_enabled_true_in_medium_with_env():
    _reset_env()
    os.environ["AJAY_DEPTH"] = "medium"
    os.environ["AJAY_PLAYWRIGHT_ENABLE"] = "1"
    _, pf = _reload_all()
...
def test_force_flag_ignores_quality_check():
    """AJAY_PLAYWRIGHT_FORCE=1 时即使数据质量高也触发."""
    _reset_env()
    os.environ["AJAY_DEPTH"] = "deep"
...
def test_autofill_summary_has_disabled_reason_when_off():
    ...
    _reset_env()
    os.environ["AJAY_DEPTH"] = "medium"  # opt-in 未设
    _, pf = _reload_all()
    summary = pf.autofill_via_playwright({"dimensions": {}}, "300308.SZ")
    assert summary["enabled"] is False
    assert "opt-in" in summary["disabled_reason"] or "AJAY_PLAYWRIGHT_ENABLE" in summary["disabled_reason"]
    _reset_env()

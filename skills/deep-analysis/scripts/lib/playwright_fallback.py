...
def is_playwright_enabled() -> bool:
    ...
    - lite (off) → 永远 False
    - medium (opt-in) → 需 AJAY_PLAYWRIGHT_ENABLE=1
    ...
    if mode == "default":
        return True
    # opt-in: 必须 env 显式启用
    return os.environ.get("AJAY_PLAYWRIGHT_ENABLE") == "1"
...
def autofill_via_playwright(raw: dict, ticker: str) -> dict:
    ...
    环境变量：
    - `AJAY_PLAYWRIGHT_FORCE=1` · 忽略 `_dim_needs_fallback`，对所有白名单维度强制跑
    ...
    if mode == "opt-in" and os.environ.get("AJAY_PLAYWRIGHT_ENABLE") != "1":
        summary["disabled_reason"] = (
            f"profile={profile.depth} · opt-in 未启用 · "
            "export AJAY_PLAYWRIGHT_ENABLE=1 启用后重跑"
        )
        print(f"   ℹ️  Playwright skip · {summary['disabled_reason']}")
        return summary
    ...
    force = os.environ.get("AJAY_PLAYWRIGHT_FORCE") == "1"

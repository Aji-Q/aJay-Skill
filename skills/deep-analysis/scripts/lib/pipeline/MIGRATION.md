...
- `AJAY_PIPELINE=1` feature flag · 默认关闭
...
- `AJAY_PIPELINE=1` feature flag 仍是 opt-in · 默认用 legacy 路径
...
- 使用方式：
  ...
  AJAY_PIPELINE=1 python3 run.py 300470.SZ --no-resume
...
### ✅ Phase 7（已完成）· run.py 接入 AJAY_PIPELINE=1 opt-in
- `run.py::main()` 顶部检测 `AJAY_PIPELINE=1` · 走 `pipeline.run_pipeline`
...
- ✅ **v3.0.0 默认启用** · `run.py` 默认走 pipeline · `AJAY_LEGACY=1` fallback
...
run.py → pipeline.run_pipeline      # 默认
       → rrt.stage1 + rrt.stage2    # AJAY_LEGACY=1 或 pipeline 异常时 fallback
```

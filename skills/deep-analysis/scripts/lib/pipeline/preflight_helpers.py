...
def prepare_target(ticker: str, *, detect_lite_fn=None) -> dict[str, Any]:
    ...
    skip_preflight = os.environ.get("AJAY_SKIP_PREFLIGHT") == "1"
    if not skip_preflight:
        try:
            ...
            # critical/degraded → 强制 lite（除非 AJAY_LITE=0 显式覆盖）
            if pre.severity in ("critical", "degraded") and os.environ.get("AJAY_LITE") != "0":
    ...
    if detect_lite_fn is not None:
        ...
        if is_lite:
            os.environ["AJAY_LITE"] = "1"  # 下游 fetcher 能读
            os.environ.setdefault("AJAY_DDG_BUDGET", "15")  # 全局 ddgs 预算上限
            ...
            print(f"   · wave3 fund_holders 默认 top 20（AJAY_FUND_LIMIT=all 可覆盖）")
            print(f"   · 全局 ddgs 预算 15 次/ticker（超出自动 skip）")
            print(f"   · 完整跑请 AJAY_LITE=0 && python run.py <ticker>\n")

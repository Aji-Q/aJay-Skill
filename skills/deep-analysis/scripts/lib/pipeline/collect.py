...
feature flag：AJAY_PIPELINE=1 时 stage1 走新管道 · 否则走老 collect_raw_data
"""
...
def is_pipeline_enabled() -> bool:
    """feature flag · 默认关 · 只在 AJAY_PIPELINE=1 时启用新管道."""
    return os.environ.get("AJAY_PIPELINE") == "1"
...
def collect(ticker: Any, raw_previous: dict | None = None, max_workers: int = 6) -> dict[str, dict]:
    ...
    if basic_dim and _is_resume_valid(basic_dim, ttl_sec=_cache_ttl(basic_fetcher)):
        ...
        t_w1 = time.time()
        fetcher_timeout = _positive_env_float("AJAY_PIPELINE_FETCHER_TIMEOUT", 120.0)
    ...
    wave_timeout = _positive_env_float("AJAY_PIPELINE_WAVE_TIMEOUT", 300.0)

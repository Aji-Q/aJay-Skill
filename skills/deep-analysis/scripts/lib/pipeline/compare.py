...
def run_dark_launch(ticker: str, save_diff_to: str | None = None) -> dict:
    ...
    完整 e2e 请在 shell 里分别跑 AJAY_PIPELINE=0 和 AJAY_PIPELINE=1 再调 `compare_cached_runs`.
    """
...
def compare_cached_runs(ticker: str, save_diff_to: str | None = None) -> dict:
    """基于已有的 cache raw_data.json 做对比（假设测试时手动切 AJAY_PIPELINE 跑了两轮）.

    实际测试流程：
    1. 用户 shell: AJAY_PIPELINE=0 python3 run.py 300470.SZ --no-resume （记下 raw_data）
    2. 用户 shell: cp .cache/300470.SZ/raw_data.json /tmp/legacy_raw.json
    3. 用户 shell: AJAY_PIPELINE=1 python3 run.py 300470.SZ --no-resume （记下 raw_data）

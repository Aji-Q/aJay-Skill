...
第一次运行：
1. 检查持久化目录 `~/.ajay-skill/playwright-xueqiu/` 是否含 XueQiu cookie
...
PROFILE_DIR = Path.home() / ".ajay-skill" / "playwright-xueqiu"
...
def is_login_enabled() -> bool:
    ...
    return os.environ.get("AJAY_XQ_LOGIN") == "1"
...
def _interactive_login() -> bool:
    ...
    if not sys.stdin.isatty():
        ...
        print("   解决：在交互式终端运行一次：")
        print("     AJAY_XQ_LOGIN=1 python -m lib.xueqiu_browser login")
        return False
    try:
...
def fetch_with_browser(url: str, timeout: int = 15) -> str | None:
    ...
    if not PROFILE_DIR.exists():
        print(f"   ℹ️ XueQiu 未登录 (AJAY_XQ_LOGIN=1 但首次需跑 `python -m lib.xueqiu_browser login`)")
        return None
    try:
        from playwright.sync_api import sync_playwright  # type: ignore
    except ImportError:
...
if __name__ == "__main__":
    ...
    if cmd == "login":
        os.environ["AJAY_XQ_LOGIN"] = "1"  # ensure enabled for this run
        ...
        print()
        print("Setup steps:")
        print(f"  1) export AJAY_XQ_LOGIN=1")

...
使用:
  ...
  profile = get_profile()         # 从 AJAY_DEPTH env 读，默认 medium
...
CLI 集成（run.py）:
  ...
  AJAY_DEPTH=deep python run.py 600519

兼容:
  - AJAY_LITE=1 向后兼容 → 等价于 AJAY_DEPTH=lite
...
class AnalysisProfile:
    ...
    #   'off'      → 完全禁用（lite · 保持快扫）
    #   'opt-in'   → AJAY_PLAYWRIGHT_ENABLE=1 / --enable-playwright 才启用（medium）
    #               · 未装时打印安装命令，本次跳过（不自动装）
...
_PROFILES: dict[str, AnalysisProfile] = {
    ...
    DEPTH_MEDIUM: AnalysisProfile(
        ...
        self_review_block_warnings=False,
        playwright_mode="opt-in",          # v2.13.0 · AJAY_PLAYWRIGHT_ENABLE=1 启用 · 未装时提示命令
...
def get_profile(depth: str | None = None) -> AnalysisProfile:
    ...
    参数:
      depth: 显式指定 lite/medium/deep；None 时从 AJAY_DEPTH env 读（默认 medium）

    向后兼容:
      AJAY_LITE=1 → 等价 depth=lite
      AJAY_LITE=0 → 等价 depth=medium（不强制 deep）
    """
    if depth is None:
        depth = os.environ.get("AJAY_DEPTH")
        # 兼容老的 AJAY_LITE
        if depth is None:
            lite_env = os.environ.get("AJAY_LITE", "auto").lower()
...
def apply_profile_to_env(profile: AnalysisProfile) -> None:
    ...
    os.environ["AJAY_DEPTH"] = profile.depth
    os.environ["AJAY_LITE"] = "1" if profile.depth == DEPTH_LITE else "0"
    os.environ["AJAY_DDG_BUDGET"] = str(profile.ddg_budget) if profile.ddg_budget > 0 else "0"
    os.environ["AJAY_FUND_STATS_TOP"] = str(profile.fund_stats_top_n)

...
def render_council(raw, analysis=None):
    ...
    html=(ASSETS/'report-council.html').read_text().replace('{{PAGE_TITLE}}',escape(str(data['security']['name'] or 'aJay')+' · aJay Investment Council'))
    ...
    fallback='<noscript><main class=\"noscript\"><h1>aJay 研究会议</h1><p>此交互报告需要启用 JavaScript。原始研究输入保留如下：</p><pre>'+escape(json.dumps(json_safe(raw),ensure_ascii=False,indent=2))+'</pre></main></noscript>'

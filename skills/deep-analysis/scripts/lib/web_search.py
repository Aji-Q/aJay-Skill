...
# ═══════════════════════════════════════════════════════════════
# AJAY_LITE=1 时 search() 进入严格预算模式：全生命周期最多 N 次真实搜索
...
def _budget_allows() -> bool:
    import os
    cap_raw = os.environ.get("AJAY_DDG_BUDGET")
    if not cap_raw:
        return True  # 未设预算 = 无限
    try:
        cap = int(cap_raw)
...
def _ddg_search(query: str, max_results: int = 10, region: str = "cn-zh") -> list[dict]:
    ...
    # AJAY_DDG_TIMEOUT 可调（默认 10 秒）
    import os, concurrent.futures as _cf
    timeout_sec = int(os.environ.get("AJAY_DDG_TIMEOUT", "10"))
...
def search(query: str, max_results: int = 10, cache_key_prefix: str = "ws") -> list[dict]:
    ...
    v2.10.1 · 命中 cache 的不占预算；未命中 cache 时检查 AJAY_DDG_BUDGET 预算。
    """
    ...
    def _fetcher():
        if not _budget_allows():
            ...
            return [{"_budget_exceeded": True,
                     "body": "全局 ddgs 预算已用尽（AJAY_DDG_BUDGET），agent 请用 cached / hardcoded 数据"}]
        _budget_mark_used()

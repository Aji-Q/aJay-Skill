    ...
    if args.output_dir:
        ...
        try:
            ...
            meta = {
                "schema": 1,
                "ticker": args.ticker,
                "depth": args.depth or os.environ.get("AJAY_DEPTH") or "medium",
...
def main():
    parser = argparse.ArgumentParser(
        description="游资（aJay）Skills · 个股深度分析",
    ...
    # v2.10.2 · 深度选择（优先级: --depth > AJAY_DEPTH env > AJAY_LITE env > 默认 medium）
    try:
    ...
    if os.environ.get("AJAY_DEPTH") == "deep":
        os.environ.pop("AJAY_CLI_ONLY", None)
    else:
        os.environ.setdefault("AJAY_CLI_ONLY", "1")
    ...
    print()
    print("━" * 50)
    print(f"🎯 游资（aJay）Skills v{_get_version()} · 深度分析引擎")
    ...
    _force_legacy = os.environ.get("AJAY_LEGACY") == "1"
    ...
    if _modeling_succeeded:
        ...
        if isinstance(stage1_result, dict) and stage1_result.get("status") == "non_stock_security":
            ...
            if sec_type in ("etf", "lof", "mutual_fund") and stage1_result.get("top_holdings"):
                ...
                auto_yes = os.environ.get("AJAY_FUND_AUTO_YES") == "1"
